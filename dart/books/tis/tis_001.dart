...
class Pronoun {
  ...
  static final Pronoun YOU = const Pronoun("you", "you", "your", "yourself");
  static final Pronoun HE = const Pronoun("he", "him", "his", "himself");
  static final Pronoun SHE = const Pronoun("she", "her", "her", "herself");
  static final Pronoun IT = const Pronoun("it", "it", "its", "itself");
}
...
class Storyline {
  ...
  static String resolveRandoms(String str) {
    ...
    if (startTagIndex != -1 && startTagIndex < str.length - 1) {
      ...
      if (numOptions > 1) {
        int choice = (Math.random() * numOptions).floor().toInt();
...
class CombatMoveType extends GameEntity {
  ...
  static final int MOVE_TRG_WAIST = 1<<5;
  static final int MOVE_TRG_LEGS = 1<<6;
  static final int MOVE_TRG_ALL = MOVE_TRG_HEAD|MOVE_TRG_NECK|MOVE_TRG_ARMS|MOVE_TRG_BODY|MOVE_TRG_WAIST|MOVE_TRG_LEGS;

  // directions of the move
  static final int MOVE_FRM_FRONT = 1<<7;
  static final int MOVE_FRM_BEHIND = 1<<8;
  static final int MOVE_FRM_LEFT = 1<<9;
  static final int MOVE_FRM_RIGHT = 1<<10;
  ...
  static final int MOVE_LMB_HAND = 1<<11;
  static final int MOVE_LMB_LEG = 1<<12;
  static final int MOVE_LMB_HEAD = 1<<13;
  static final int MOVE_LMB_SPECIAL = 1<<14; // e.g. tail

  // move damages {hitpoints|stance}?
  static final int MOVE_EFF_HITPOINTS = 1<<15;
  static final int MOVE_EFF_STANCE = 1<<16;
  static final int MOVE_EFF_ALL = MOVE_EFF_HITPOINTS|MOVE_EFF_STANCE;

  // priority: quicker or more damage?
  static final int MOVE_PRI_QUICK = 1<<17;
  static final int MOVE_PRI_DAMAGE = 1<<18;

  // defensive or offensive move
  static final int MOVE_DIR_OFFENSIVE = 1<<19;
  static final int MOVE_DIR_DEFENSIVE = 1<<20;
...
class Combat extends GameEntity implements LoopedEvent {
  ...
  static final int MAX_MOVES_PRESENTED = 50;
...
class ScripterImpl extends Scripter {
  ...
  ScripterImpl() : super() {
    pages = [
      ...
      [
        ...
        () {
          vars["man"] = new Actor();
          vars["man"].moves.addAll(vars["humanMoves"]);
          vars["man"].weapon = vars["weaponHands"];
          vars["man"].fighting = 0;
          vars["man"].names = ["the drunkard", "the drunk man", "the man"];
          vars["man"].on["sufferHit"].add((_this, e) { _this.report("<subject> looks very surprised"); });
          vars["man"].armors.add(vars["armorHelmet"]);
        },
        "He charges at you!",
        () {
          vars["combat"] = new Combat();
          vars["combat"].actors.addAll([ vars["player"], vars["man"] ]);
      ...
      [
        ...
        () {
          ...
          Actor b = new Actor();
          b.stance = 19;
          assert(vars["moveKickLegs"].canContinue(a, b) == false);
          assert(vars["moveKickLegs"].isApplicable(a, b) == false);
      ...
      [
        ...
        () {
          vars["NUMBER_OF_RUNS"] = 10;
          vars["teamOneWins"] = 0;
          vars["teamTwoWins"] = 0;
        
          Function runTest = () {
            vars["swarmer1"] = new Actor();
            vars["swarmer1"].names = ["the first swarmer"];
            vars["swarmer1"].pronoun = Pronoun.IT;
            vars["swarmer1"].moves.addAll(vars["humanMoves"]);
            vars["swarmer1"].team = 2;
        
            vars["swarmer2"] = new Actor();
            vars["swarmer2"].names = ["the second swarmer"];
            vars["swarmer2"].pronoun = Pronoun.IT;
            vars["swarmer2"].moves.addAll(vars["humanMoves"]);
            vars["swarmer2"].team = 2;
        
            vars["swarmer3"] = new Actor();
            vars["swarmer3"].names = ["the third swarmer"];
            vars["swarmer3"].pronoun = Pronoun.IT;
            vars["swarmer3"].moves.addAll(vars["humanMoves"]);
            vars["swarmer3"].team = 2;
        
            vars["individual1"] = new Actor();
            vars["individual1"].names = ["the first individual"];
            vars["individual1"].moves.addAll(vars["humanMoves"]);
            vars["individual1"].team = 3;
        
            vars["individual2"] = new Actor();
            vars["individual2"].names = ["the second individual"];
            vars["individual2"].moves.addAll(vars["humanMoves"]);
            vars["individual2"].team = 3;
        
            vars["combat"] = new Combat();
            vars["combat"].actors.addAll([vars["individual1"], vars["swarmer1"], vars["individual2"], vars["swarmer2"], vars["swarmer3"]]);
            vars["combat"].on["update"].add((combat) {
              if ((combat.time % 10) == 0)
                combat.storyline.add("it's 10 seconds later");
            });
            vars["combat"].start();
            vars["combat"].updateUntilInteraction();
        
            assert(vars["combat"].finished);
            assert((!vars["swarmer1"].alive && !vars["swarmer2"].alive && !vars["swarmer3"].alive) 
                  || (!vars["individual1"].alive && !vars["individual2"].alive));
        
            DEBUG(vars["combat"].storyline.toString());
        
            if (vars["individual1"].alive || vars["individual2"].alive)
              vars["teamTwoWins"]++;
            else
              vars["teamOneWins"]++;
          };
        
          for (int i = 0; i < vars["NUMBER_OF_RUNS"]; i++)
            runTest();
          DEBUG("Swarm vs Individual: ${vars['teamOneWins']} : ${vars['teamTwoWins']}");
          assert(vars["teamOneWins"] > vars["teamTwoWins"]);
  ...
  void initBlock() {
      ...
      // move: hit to stomach
      vars["moveStomachPunch"] = new CombatMoveType(
      ...
      vars["moveStomachPunch"].isApplicable 
          = vars["moveStomachPunch"].canContinue 
          = (performer, target) => performer.stance >= 20 && target.stance >= 20; 
    
      // move: right hook
      vars["moveRightHook"] = new CombatMoveType(
      ...
      vars["moveRightHook"].isApplicable 
          = vars["moveRightHook"].canContinue 
          = (performer, target) => performer.stance >= 20 && target.stance >= 10; 
    
      vars["moveRightHook"].on["hit"].addFirst((_this, e) {
          e.performer.report("{Boom|Thump}!", wholeSentence:true);
      });
    
      // move: jab
      vars["moveJab"] = new CombatMoveType(
          flags: CombatMoveType.MOVE_TRG_BODY|CombatMoveType.MOVE_FRM_FRONT
      ...
      vars["moveJab"].isApplicable 
          = vars["moveJab"].canContinue 
          = (performer, target) => performer.stance >= 20 && target.stance >= 10; 
    
    
      // move: punch in groin
      vars["moveGroinPunch"] = new CombatMoveType(
      ...
      vars["moveGroinPunch"].isApplicable 
          = vars["moveGroinPunch"].canContinue 
          = (performer, target) => performer.stance >= 10 && target.stance >= 20; 
    
      // move: kick legs
      vars["moveKickLegs"] = new CombatMoveType(
      ...
      vars["moveKickLegs"].isApplicable 
          = vars["moveKickLegs"].canContinue 
          = (performer, target) => performer.stance >= 30 && target.stance >= 20;
    
      // move: kick in groin
    
      // move: undercut legs
      vars["moveFootSweep"] = new CombatMoveType(
          flags: CombatMoveType.MOVE_TRG_LEGS|CombatMoveType.MOVE_FRM_RIGHT
                |CombatMoveType.MOVE_LMB_LEG|CombatMoveType.MOVE_EFF_STANCE
                |CombatMoveType.MOVE_PRI_DAMAGE|CombatMoveType.MOVE_DIR_OFFENSIVE,
          string: "foot sweep",
      ...
      vars["moveFootSweep"].on["hit"].add((_this, e) {
      ...
      vars["moveFootSweep"].isApplicable = vars["moveFootSweep"].canContinue = (performer, target) {
        return performer.stance >= 10 && target.stance >= 20;
      };
      
      // move: kick on ground
      vars["moveKickOnGround"] = new CombatMoveType(
          flags: CombatMoveType.MOVE_TRG_BODY|CombatMoveType.MOVE_FRM_FRONT
      ...
      vars["moveKickOnGround"].isApplicable 
          = vars["moveKickOnGround"].canContinue = (performer, target) {
        return performer.stance >= 20 && target.stance < 20;
      };
    
      // move: defend
      vars["moveStepBack"] = new CombatMoveType(
      ...
      vars["moveStepBack"].isApplicable = vars["moveStepBack"].canContinue 
          = (performer, target) => performer.stance >= 20;
    
      vars["moveStepBack"].on["start"].replaceAllWith((_this, e) {
          e.performer.report(_this.thirdPartyString);
      });
    
      vars["moveStepBack"].on["end"].replaceAllWith((_this, e) {
          e.performer.stance += 10;
      });
    
      // move: counter
      vars["moveBasicParryHand"] = new CombatMoveType(
      ...
      vars["moveBasicParryHand"].isApplicable = (performer, target) {
      ...
      vars["moveBasicParryHand"].canContinue = (performer, target) {
        return performer.stance >= 10 && target.currentMove != null;
      };
    
      vars["moveBasicParryHand"].on["end"].add((_this, e) {
      ...
      vars["moveBasicParryHand"].computeSuitability = (Actor performer, Actor target) {
      ...
      // move: stand up
      vars["moveStandUp"] = new CombatMoveType(
          flags: CombatMoveType.MOVE_DIR_DEFENSIVE|CombatMoveType.MOVE_LMB_LEG,
          string: "stand up",
      ...
      vars["moveStandUp"].isApplicable 
          = (performer, target) => performer.stance < 20;
    
      vars["moveStandUp"].on["update"].add((_this, e) {
      ...
      vars["moveStandUp"].on["start"].replaceAllWith((_this, e) {
          String again = (e.performer.previousMove == _this) ? " again" : "";
          e.performer.report("<subject> {gather<s>|begin<s>|tr<ies>} to stand up$again");
      });
    
      vars["moveStandUp"].on["end"].replaceAllWith((_this, e) {
          e.performer.stance += 10;
      });
    
      vars["moveStandUp"].computeSuitability = (Actor performer, Actor target) {
        return 100; // when this move is isApplicable, it's probably a good idea to use it
      };
    
      // move: roll out
      vars["moveRollOut"] = new CombatMoveType(
      ...
      vars["moveRollOut"].isApplicable 
          = (performer, target) => performer.stance < 10;
    
      vars["moveRollOut"].on["end"].replaceAllWith((_this, e) {
      ...
      vars["moveRollOut"].computeSuitability = (Actor performer, Actor target) {
        return 100; // when this move is isApplicable, it's probably a good idea to use it
      };
      ...
      vars["moveSlash"] = new CombatMoveType(
      ...
      vars["humanMoves"] = [
          vars["moveStomachPunch"],
          vars["moveRightHook"],
          vars["moveJab"],
          vars["moveKickLegs"],
          vars["moveFootSweep"],
          vars["moveKickOnGround"],
          vars["moveStepBack"],
          vars["moveStandUp"],
          vars["moveBasicParryHand"],
          vars["moveGroinPunch"],
          vars["moveRollOut"]
      ];
      ...
      // weapon: bare hands
      vars["weaponHands"] = new WeaponType(
          name: "hand",
          piercing: 0,
          hardness: 0
      );
      ...
      // weapon: sword
      vars["weaponSword"] = new WeaponType(
          name: "sword",
          piercing: 3,
          hardness: 3
      );
      ...
      // armor: clothes
      vars["armorClothes"] = new ArmorType(
          name: "clothes",
          hardness: 0
      );
    
      vars["armorHelmet"] = new ArmorType(
          name: "helmet",
          hardness: 1,
      ...
      vars["player"].weapon = vars["weaponHands"];
      //vars["player"].weapon = vars["weaponSword"];
