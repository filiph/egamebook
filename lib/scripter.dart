...
abstract class Scripter extends ScripterViewedFromPresenter {
  ...
  void walk() {
    ...
    do {
      ...
      try {
        ...
        presenter.reportError("AuthorScriptException",
  ...
  void restart() {
    ...
    presenter.setStats(UIStat.overwriteStatsListFromDataStructure(Stat.createStatList()));
  ...
  bool _goOneStep() {
    if (!_points.pointsAwards.isEmpty) {
      ...
      presenter.awardPoints(new PointsAward(award.addition, award.result,
          award.justification))
      .then((_) => walk());
      return _STOP;
    }
    ...
    if (_currentForm != null) {
      ...
      subscription = stream.listen((CurrentState values) {
        ...
        try {
          ...
          presenter.reportError("Error while handling user input in Form",
              "$e\n$stacktrace");
          throw e;
        }
        if (!values.submitted) {
    ...
    if (currentPage.blocks[currentBlockIndex] is String) {
      ...
      presenter.showText(currentPage.blocks[currentBlockIndex])
      .then((_) => walk());
  ...
  void _initScriptEnvironment() {
    ...
    try {
      ...
      presenter.reportError("Author Exception in initBlock() (<variables>)",
          "$e\n$stacktrace");
      throw e;
    }
