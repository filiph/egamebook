...
builderReadyHandler(Builder b) {
  if (graphInput) {
    print(
        "Warning: updateFromGraphMLFile currently not implemented."); //b.updateFromGraphMLFile();
  ...
  if (graphOutput) {
    print(
        "Warning: writeGraphMLFile currently not implemented."); //b.writeGraphMLFile();
  }
  if (compile) {
...
void main(List<String> args) {
  ...
  if (results.rest.length < 1) {
    print(
        "Script called without file argument. Please provide file to work on.");
    print(parser.usage);
    return;
  }
  var filename = results.rest[0];

  new Builder().readEgbFile(new File(filename)).then(builderReadyHandler);
}
